...
- 调用方法
   ...
   - 对于可自定义选项卡的新版本，可以在*文件 -> 选项 -> 自定义功能区* 中将宏添加到选项卡中
   - 以上两种方案操作，[具体操作参考这里](https://jingyan.baidu.com/article/4dc40848753509c8d946f1a7.html)
	- 在开发工具，插入按钮控件并且绑定宏（开发工具选项卡可以在excel选项中找到）
- 宏功能说明
...
通过``` Application.InputBox(prompt:="选择图片位置", Type:=8)``` 指定图片黏贴位置
...
tips: 不能保护需要导入图片的工作表，会导致图片无法插入（仅部分工作表会有bug？？？）
